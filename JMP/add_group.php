...
<?php
  if(isset($_POST['add'])){
   ...
   if(find_by_groupName($_POST['group-name']) === false ){
     $session->msg('d','Entered Group Name is already in the database.');
     redirect('add_group.php', false);
   }elseif(find_by_groupLevel($_POST['group-level']) === false) {
     $session->msg('d','Entered Group Level is already in the database.');
     redirect('add_group.php', false);
   }
   if(empty($errors)){
        ...
        if($db->query($query)){
          ...
          //failed
          $session->msg('d',' Group failed to create.');
          redirect('add_group.php', false);
        }
   } else {
